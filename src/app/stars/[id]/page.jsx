...
const StarPage = ({params}) => {
    ...
    const discovererElement = discoverer ?
        <>
            One of the most notable figures in {name} exploration is <Link href={`/discoverers/${discovererId}`}>{discoverer.name} {discoverer.occupation}.</Link>
        </>
    ...
    if (system) {
        ...
        systemElement =
            <>
                {" " + name} belongs to the <Link href={`/systems/${systemId}`}>{system.name}</Link> {starsElement} system.
            </>
    }
